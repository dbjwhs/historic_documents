...
**Author:** Danny Cohen (USC/ISI)
**Date:** April 1, 1980
**Type:** Internet Engineering Note (IEN)

https://www.rfc-editor.org/ien/ien137.txt

This seminal document addresses the fundamental computer science problem of **endianness** - the order in which bits and bytes are transmitted in serial communications. Cohen frames this technical debate as a "holy war" between two camps, drawing a clever analogy to Jonathan Swift's Gulliver's Travels.

### The Core Problem
When transmitting multi-byte data serially, there are two fundamental approaches:
- **Little-Endian**: Transmit least significant bit/byte first
- **Big-Endian**: Transmit most significant bit/byte first

### Key Technical Insights
- **Memory vs. Transmission**: Most computer systems have inconsistent internal organization, mixing endianness conventions across different data types
- **Communication Compatibility**: The choice becomes critical for interoperability between systems with different architectures
- **Standards Evolution**: Communication hardware was already predominantly Little-Endian (RS232, HDLC), suggesting the eventual winner

### Historical Significance
This document predicted many networking compatibility issues that became crucial as computer networks expanded. The "byte order problem" remains relevant in modern network protocols, file formats, and cross-platform development. Cohen's analysis helped establish the importance of standardization over technical superiority.

### Conclusion
Cohen argues that while the technical choice is somewhat arbitrary, **universal agreement on a single standard is essential** to prevent communication chaos. The document combines rigorous technical analysis with wit, making complex concepts accessible while highlighting how technical standards emerge through practical necessity rather than pure logic.
...
How about tossing a coin ???
 ...
 Figure 1: Possible orders, consistent: (1)+(2), inconsistent: ()+(4).
